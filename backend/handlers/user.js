...
const getAll = async (queryPayload) => {
  Object.keys(queryPayload).forEach((key) =>
    queryPayload[key] === undefined ? delete queryPayload[key] : {}
  );
...
const removeSummaryFile = async (payload) => {
  ...
  updatedUser = await User.findByIdAndUpdate(
    userId,
    {
      $pull: { summaryFiles: { _id: payload._id } },
    },
    { new: true }
  );
  return updatedUser;
...
const addFeedback = async (payload) => {
  ...
  const user = await User.findOne({ _id: userId });
  if (!user) {
    throw Boom.badRequest("This User does not exist");
  }
  ...
  if (feedback.feedbackType === "manuscript") {
    user.manuscriptFeedbacks.push(feedback);
  } else {
    user.summaryFeedbacks.push(feedback);
  }
  await user.save();
  return feedback;
};
